...
async function getTasksList(queryParams = {}) {
  try {
    ...
    if (platform) {
      filteredTasks = filteredTasks.filter(task => task.platforms.includes(platform));
    }
    if (recruitmentStatus) {
      filteredTasks = filteredTasks.filter(task => task.recruitmentStatus === recruitmentStatus);
    }
    if (search) {
